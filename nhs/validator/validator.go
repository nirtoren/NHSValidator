...
func multiplyByFactors(nhs string, mapping map[int]int, resultChan chan<- int, wg *sync.WaitGroup){
	defer wg.Done()
	...
	for i, digitChar := range nhs {
		...
		// Calculate the factor for the current position (1-based index)
		factor := mapping[i+1]
...
func (vc *ValidationChain) ProcessMap(nhs string) (int, error) {
	...
	wg.Add(1)
	go multiplyByFactors(nhs, vc.factorsPerDigits, resultChan, &wg)
